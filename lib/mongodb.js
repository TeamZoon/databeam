...
export class MongoDataBeam extends DataBeam {
	...
	initialize() {
		...
		this.socket.on('read', function (packet) {
			if (!!packet.id) {
				self.collection.findOne({ _id: ObjectID(packet.id) }, (err, doc) => {
					if (!!err) {
						this.emit('error', {
							event: 'read',
							data: err
		...
		this.socket.on('update', function (packet) {
			self.collection.findOneAndUpdate({ _id: ObjectID(packet.id) }, { $set: packet.data }, { returnOriginal: false }, (err, result) => {
				if (!!err) {
					this.emit('error', {
						event: 'update',
						data: err
		...
		this.socket.on('delete', function (packet) {
			self.collection.findOneAndDelete({ _id: ObjectID(packet.id) }, (err, result) => {
				if (!!err) {
					this.emit('error', {
						event: 'delete',
						data: err
